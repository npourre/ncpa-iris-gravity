...
def extract_ncpa_iris(cube_1ut, nZ, nRtcMod, nRtcPause, modAmp, repeat):
...
if __name__ == '__main__':
    ...
    filename = args.name_acquisition + '_DIT.fits'
    ...
    for iTel in telescopes:
        ...
        ncpas, iStart, iStop, nMeasure, nPause, threshold, nSample, mix_f = extract_ncpa_iris(cube_1ut, nZ, nRtcMod, nRtcPause, modAmp,args.repeat)
    ...
    if args.silent==0:
        ...
        if args.tel==0: #4 UTs at once
            fig, axarr = plt.subplots(2, 2, figsize=(12,8))
            for indTel in range(4):
                for iR in range(args.repeat):
                    for iZ in range(len(iZs)):
                        iter_length = int((iStopArr[indTel] - iStartArr[indTel])/args.repeat)
            ...
            fig, axarr = plt.subplots(1, 1, figsize=(8,6))
            iter_length = int((iStopArr[0] - iStartArr[0])/args.repeat)
            for iR in range(args.repeat):
                for iZ in range(len(iZs)):
