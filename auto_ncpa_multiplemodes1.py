...
if __name__ == '__main__':
    ...
    print("CORRECTING NOLL {0} on UT{1} with {2} repetitions for the modulation, {3} seconds per modulation element".format(args.mode_start,ut_str,args.mode_end,args.timepermode))
    ...
    if args.inst == "IRIS":
        ...
        duration_acq = (((args.timepermode+0.5) * (args.mode_end-args.mode_start+1)) + 1.5)*args.repeat + 18 
        os.system('python 2_iris_ncpa_multiplemodes.py {0} {1} {2} {3} {4} -d {5} -b {6}'.format(args.tel, args.mode_start, args.mode_end, args.repeat, args.floop, name_acquisition, duration_acq, args.background ))
    elif args.inst == "GRAV":
    ...
    if args.inst == "IRIS":
        ...
        print("################")          
        os.system('python 3_process_ncpa_iris_multiplemodes.py {0} {1} {2} {3} {4} -t {5} -s {6}'.format(args.tel, args.mode_start, args.mode_end, args.repeat, args.floop, name_acquisition, args.timepermode, args.silent))
    elif args.inst == "GRAV":
        #Wait for file on the remote server
        start_time = time.time()
    ...
    while not os.path.exists('/vltuser/iss/temp_ncpa/NCPA_{0}.npy'.format(name_acquisition)):
        ...
        if (time.time()-start_time) > timeout_time:
            raise RuntimeError('Maximal waiting time reached')
    np.save('names_acqs.npy',np.array(names_acqs))
    os.system('python 4_apply_ncpa_multiplemodes.py {0} {1} {2} {3} -u {4}'.format(args.tel, args.mode_start, args.mode_end, names_acqs[0], args.user_input))
    time.sleep(1)
    if args.psf_display==1:
