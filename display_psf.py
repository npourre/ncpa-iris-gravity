...
if __name__ == '__main__':
	...
	download_latest_file(server, remote_dir, 'IrisAcq_beforecorr_{0}_DIT.fits'.format(args.tstart), temp_folder)
	download_latest_file(server, remote_dir, 'IrisAcq_aftercorr_{0}_DIT.fits'.format(args.tstart), temp_folder)
	
	#Check existence of fits files on ISS
	timeout_time = 120
	start_time = time.time()
	while not (os.path.exists('{0}IrisAcq_beforecorr_{1}_DIT.fits'.format(temp_folder, args.tstart)) and os.path.exists('{0}IrisAcq_aftercorr_{1}_DIT.fits'.format(temp_folder, args.tstart)) ):
		time.sleep(1)
	...
	iris_before = fits.getdata( temp_folder+'IrisAcq_beforecorr_{0}_DIT.fits'.format(args.tstart), 0 )
	iris_after = fits.getdata( temp_folder+'IrisAcq_aftercorr_{0}_DIT.fits'.format(args.tstart), 0 )
	...
	if args.tel==0: #4 UTs at once
		...
		plt.tight_layout()
		if args.silent==1:
			plt.savefig('IrisAcq_PSFs_{0}.png'.format(args.tstart))
		else:
			plt.show()
	elif args.tel in [1,2,3,4]: #one UT measurement
		iris_before_mean = cutIrisDet(iris_before,args.tel)[10:20].mean(0)
		iris_after_mean = cutIrisDet(iris_after,args.tel)[10:20].mean(0)
		vmax = np.max(iris_after_mean)
		norm = matplotlib.colors.LogNorm(vmax=vmax,vmin=vmax*1e-4)
